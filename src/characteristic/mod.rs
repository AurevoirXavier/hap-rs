...
pub struct Characteristic<T: fmt::Debug + Default + Clone + Serialize + Send + Sync> {
	/// Instance ID; integer assigned by the server to uniquely identify the HAP characteristic
	/// object.
	id: u64,
	...
	perms: Vec<Perm>,
	/// String describing the characteristic on a manufacturer-specific basis, such as an indoor
	/// versus outdoor temperature reading.
	description: Option<String>,
	/// Boolean indicating if event notifications are enabled for this characteristic.
	event_notifications: Option<bool>,

	/// The value of the characteristic, which must conform to the `format` property. This property
	/// must be present if and only if the characteristic contains the Paired Read permission.
	value: T,
	/// Unit of the value, e.g. `Celsius`.
	unit: Option<Unit>,

	/// Maximum value for the characteristic, which is only appropriate for characteristics that
	/// have a format of `Int` or `Float`.
	max_value: Option<T>, // TODO - use this value in `set_value`
	// Minimum value for the characteristic, which is only appropriate for characteristics that
	// have a format of `Int` or `Float`.
	min_value: Option<T>, // TODO - use this value in `set_value`
	/// Minimum step value for the characteristic, which is only appropriate for characteristics
	/// that have a format of ”int” or ”float”. For example, if this were 0.15, the characteris-
	/// tic value can be incremented from the min- imum value in multiples of 0.15. For “float”,
	/// the “Value” needs to be rounded on the ac- cessory side to the closest allowed value per
	/// the ”Step Value” (e.g. a value of 10.150001 received on the accessory side with a
	...
	/// Maximum number of characters if the for- mat is ”string”. If this property is omitted for
	/// ”string” formats, then the default value is 64. The maximum value allowed is 256.
	max_len: Option<u16>, // TODO - use this value in `set_value`
	/// Maximum number of characters if the format is ”data”. If this property is omitted for
	/// ”data” formats, then the default value is 2097152.
	...
	/// A 2 element array representing the starting value and ending value of the range of valid
	/// values.
	valid_values_range: Option<[T; 2]>, // TODO - use this value in `set_value`

	/// Specified TTL in milliseconds the controller requests the accessory to securely execute a
	/// write command. Maximum value of this is 9007199254740991.
	...
	/// Sets a callback function on a characteristic that is called every time a controller
	/// attempts to read its value. Returning a `Some(T)` from this function changes the value of
	/// the characteristic before the controller reads it so the Controller reads the new value.
	on_read: Option<Box<dyn OnReadFn<T>>>,
	/// Sets a callback function on a characteristic that is called every time a controller
	/// attempts to update its value. The first argument is a reference to the current value of
	/// the characteristic and the second argument is a reference to the value the controller
	/// attempts to change the characteristic's to.
	on_update: Option<Box<dyn OnUpdateFn<T>>>,
	/// Sets an async callback function on a characteristic that is driven to completion by the
	/// async runtime driving the HAP server every time a controller attempts to read its value.
	/// Returning a `Some(T)` from this function changes the value of the characteristic before
	/// the controller reads it so the controller reads the new value.
	on_read_async: Option<Box<dyn OnReadFuture<T>>>,
	/// Sets an async callback function on a characteristic that is driven to completion by the
	/// async runtime driving the HAP server every time a controller attempts to update its value.
	/// The first argument is a reference to the current value of the characteristic and the
	/// second argument is a reference to the value the controller attempts to change the
	/// characteristic's to.
	...
	for<'de> T: Deserialize<'de>,
{
	...
	pub async fn set_value(&mut self, val: T) -> Result<()> {
		...
		if let Some(ref mut on_update_async) = self.on_update_async {
			on_update_async(old_val, val.clone()).await.map_err(Error::ValueOnUpdate)?;
		}
	...
	/// Sets a callback function on a characteristic that is called every time a controller attempts
	/// to read its value. Returning a `Some(T)` from this function changes the value of the
	...
	/// Sets a callback function on a characteristic that is called every time a controller attempts
	/// to update its value. The first argument is a reference to the current value of the
	/// characteristic and the second argument is a reference to the value the controller attempts
	...
	/// Sets an async callback function on a characteristic that is driven to completion by the
	/// async runtime driving the HAP server every time a controller attempts to read its value.
	/// Returning a `Some(T)` from this function changes the value of the characteristic before the
	/// controller reads it so the controller reads the new value.
	...
	/// Sets an async callback function on a characteristic that is driven to completion by the
	/// async runtime driving the HAP server every time a controller attempts to update its value.
	/// The first argument is a reference to the current value of the characteristic and the second
	/// argument is a reference to the value the controller attempts to change the characteristic's
	/// to.
...
/// [`HapCharacteristicSetup`](HapCharacteristicSetup) is implemented by every HAP characteristic to
/// provide helper methods used by the HAP server for setup purposes. It's not meant to be used by a
...
/// [`OnReadFn`](OnReadFn) represents a callback function to be set on a characteristic that is
/// called every time a controller attempts to read its value. Returning a `Some(T)` from this
/// function changes the value of the characteristic before the controller reads it so the
...
/// [`OnUpdateFn`](OnUpdateFn) represents a callback function to be set on a characteristic that is
/// called every time a controller attempts to update its value. The first argument is a reference
/// to the current value of the characteristic and the second argument is a reference to the value
/// the controller attempts to change the characteristic's to.
...
/// [`OnReadFuture`](OnReadFuture) represents an async callback function to be set on a
/// characteristic that is driven to completion by the async runtime driving the HAP server every
/// time a controller attempts to read its value. Returning a `Some(T)` from this function changes
/// the value of the characteristic before the controller reads it so the controller reads the new
/// value.
...
/// [`OnUpdateFuture`](OnUpdateFuture) represents an async callback function to be set on a
/// characteristic that is driven to completion by the async runtime driving the HAP server every
/// time a controller attempts to update its value. The first argument is a reference to the current
/// value of the characteristic and the second argument is a reference to the value the controller
/// attempts to change the characteristic's to.
...
/// Trait containing the [`OnReadFuture`](OnReadFuture) and [`OnUpdateFuture`](OnUpdateFuture)
/// callback functions.
	...
	for<'de> T: Deserialize<'de>,
{
	...
	async fn set_value(&mut self, value: serde_json::Value) -> Result<()> {
		let v;
		// for whatever reason, the controller is setting boolean values either as a boolean or as
		// an integer
