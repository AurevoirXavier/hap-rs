...
pub struct Config {
	/// Socket IP address to serve on. Defaults to the IP of the system's first non-loopback
	/// network interface.
	pub host: IpAddr,
	...
	/// the accessory's Pairing Identifier. Must be a unique random number generated at every
	...
	/// added or removed on the accessory server. Accessories must increment the config number
	...
	/// Protocol version string `<major>.<minor>` (e.g. `"1.0"`). Defaults to `"1.0"` Required if
	/// value is not `"1.0"`.
...
impl Config {
	/// Redetermines the `host` field to the IP of the system's first non-loopback network
	/// interface.
