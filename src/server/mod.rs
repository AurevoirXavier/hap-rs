...
/// [`Server`](Server) is implemented by the transport methods this crate supports. Currently,
/// that's just [`IpServer`](IpServer).
#[async_trait]
pub trait Server {
	/// Returns a [`BoxFuture`](BoxFuture) run handle to the server that can be passed to an
	/// executor.
	...
	// /// Every accessory must support a manufacturer-defined mechanism to restore itself to a
	// “factory reset” state where /// all pairing information is erased and restored to factory
	// default settings. This method is doing just that. async fn factory_reset(&mut self) ->
	// Result<()>;
}
