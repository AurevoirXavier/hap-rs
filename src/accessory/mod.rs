...
/// [`HapAccessorySetup`](HapAccessorySetup) is implemented by every HAP accessory to provide helper
/// methods used by the HAP server for setup purposes. It's not meant to be used by a consumer of
/// the library.
...
/// The [`AccessoryInformation`](AccessoryInformation) struct is used to conveniently store metadata
/// about an accessory and is converted to the
/// [`AccessoryInformationService`](AccessoryInformationService) of the accessory it is passed to on
/// its creation.
///
/// # Examples
///
...
pub struct AccessoryInformation {
	...
	/// - Subsequent firmware updates can have a lower <z> version only if <x> or <y> is
	///   incremented
	///
...
impl AccessoryInformation {
	...
	pub fn to_service(self, id: u64, accessory_id: u64) -> Result<AccessoryInformationService> {
		...
		executor::block_on(
			i.serial_number.set_value(serde_json::Value::String(self.serial_number)),
		)?;
