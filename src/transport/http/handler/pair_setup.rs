	...
	fn parse(&self, body: Body) -> BoxFuture<Result<Step, tlv::ErrorContainer>> {
		async {
			...
			match decoded.get(&(Type::State as u8)) {
				...
				None =>
					Err(tlv::ErrorContainer::new(StepNumber::Unknown as u8, tlv::Error::Unknown)),
			}
		}
		.boxed()
	}
...
async fn handle_start(
	...
	// TODO
	// If the accessory is currently performing a PairSetup procedure with a different controller,
	// it must respond with the following TLV items:
	// kTLVType_State <M2>
	...
	let user = UserRecord { username: b"Pair-Setup", salt: &salt, verifier: &verifier };
	let srp_server = SrpServer::<Sha512>::new(&user, b"foo", &b, &G_3072)?;
	let b_pub = srp_server.get_b_pub();

	handler.session =
		Some(Session { salt, verifier, b, b_pub: b_pub.clone(), shared_secret: None });
...
async fn handle_verify(
	...
	match handler.session {
		...
		Some(ref mut session) => {
			...
			Ok(vec![Value::State(StepNumber::SrpVerifyResponse as u8), Value::Proof(b_proof)])
		},
	}
}
		...
		Some(ref mut session) => match session.shared_secret {
			...
			Some(ref shared_secret) => {
				...
				let device_pairing_id =
					sub_tlv.get(&(Type::Identifier as u8)).ok_or(tlv::Error::Unknown)?;
				...
				event_emitter.lock().await.emit(&Event::ControllerPaired { id: pairing.id }).await;
