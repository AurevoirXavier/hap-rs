...
async fn handle_remove(
	...
	event_emitter.lock().await.emit(&Event::ControllerUnpaired { id: pairing_uuid }).await;
...
async fn check_admin(
	...
	let controller_id: Uuid =
		controller_id.read().unwrap().deref().ok_or(tlv::Error::Authentication)?;
