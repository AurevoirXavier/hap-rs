	...
	fn poll_read(
		...
		match stream_wrapper.poll_receiver(cx) {
			...
			Poll::Ready(_r_len) => {
				...
				if let Some(waker) =
					stream_wrapper.outgoing_waker.lock().expect("accessing outgoing_waker").take()
				{
					waker.wake()
				}
				if let Some(waker) =
					stream_wrapper.incoming_waker.lock().expect("accessing incoming_waker").take()
				{
					waker.wake()
				}
	...
	fn poll_outgoing(
		...
		loop {
			match Stream::poll_next(Pin::new(&mut encrypted_stream.outgoing_receiver), cx) {
				Poll::Pending => {
					*encrypted_stream.outgoing_waker.lock().expect("setting outgoing_waker") =
						Some(cx.waker().clone());
					return Poll::Pending;
				},
		...
		loop {
			match AsyncRead::poll_read(Pin::new(encrypted_stream), cx, &mut data) {
				...
				Poll::Ready(Ok(())) => {
					...
					encrypted_stream.incoming_sender.unbounded_send(data_filled.to_vec()).map_err(
						|_| io::Error::new(io::ErrorKind::Other, "couldn't send incoming data"),
					)?;
	...
	fn poll_write(
		...
		if let Some(shared_secret) = encrypted_stream.shared_secret {
			...
			let (aad, chunk, auth_tag) =
				encrypt_chunk(&shared_secret, &write_buf, &mut encrypted_stream.encrypt_count)
					.map_err(|_| io::Error::new(io::ErrorKind::Other, "encryption failed"))?;
