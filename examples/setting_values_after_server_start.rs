...
async fn main() -> Result<()> {
	let sensor = MotionSensorAccessory::new(
		1,
		AccessoryInformation { name: "Acme Sensor".into(), ..Default::default() },
	)?;
	...
	let value_set_interval = async move {
		...
		loop {
			...
			let motion_sensor_service =
				motion_sensor_accessory.get_mut_service(HapType::MotionSensor).unwrap();
			let motion_detected_characteristic =
				motion_sensor_service.get_mut_characteristic(HapType::MotionDetected).unwrap();
